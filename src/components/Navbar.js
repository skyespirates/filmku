  ...
  return (
    <div className="bg-black" style={{ backgroundColor: '#1B2124' }}>
      <div onClick={() => setToggle(!toggle)} className="relative flex flex-col px-2 py-6 mx-auto cursor-pointer max-w-7xl md:flex-row md:justify-between">
        <Link id="logo" className="text-2xl font-bold text-blue-700 md:text-4xl" style={{ color: '#04A3DD' }} to="/">
          FILMKU
        </Link>
        <div className="absolute z-10 flex items-center justify-center w-6 h-6 rounded-full md:hidden top-3 right-3">
          <ion-icon size="large" name="menu-outline"></ion-icon>
        </div>
        <ul className={`text-sm md:text-base items-center justify-center md:justify-end  w-full  md:flex ${toggle ? 'block' : 'hidden'}`}>
